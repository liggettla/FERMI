setwd('/media/alex/Extra/Dropbox/Degregori_Lab/Experiments/9.22.2016_SequencingAnalysis/H3-R1.fastq')
oncoGenes <- read.table('oncoGenes.txt', header = TRUE)
seq=c("A", "C", "T", "G")
df=data.frame('Ref'=rep(seq, each=4), 'Var'=rep(seq, 4), 'Obs'=oncoGenes$Obs)
ggplot(data=df) + aes(x=Var, y=Obs, title='H3-R1 OncoGenes') +
...
ggplot(data=df) + aes(x=Var, y=Obs, title='H3-R1 OncoSites') + 
  geom_bar(stat='identity', position="dodge", fill="grey", color="black")  +
  facet_grid(~Ref)
...
ggplot(data=df) + aes(x=Var, y=Obs, title='H3-R1 TIIIRegions') + 
  geom_bar(stat='identity', position="dodge", fill="grey", color="black")  +
  facet_grid(~Ref)